...
export default function DetailScreen() {
  ...
  return (
    <View style={s.container} testID="detail-screen">
      {/* Hero */}
      <ImageBackground source={{ uri: activity.image }} style={s.hero} imageStyle={s.heroImage}>
        <View style={s.heroOverlay} />
        <TouchableOpacity style={s.backBtn} onPress={() => router.back()} testID="detail-back-btn">
          <Feather name="chevron-left" size={16} color="#fff" />
        </TouchableOpacity>
        <TouchableOpacity style={s.favBtn} testID="detail-fav-btn">
      ...
      <ScrollView style={s.body} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.locRow}>
          ...
          <Text style={s.locText}>{activity.address} - {activity.arrondissement}</Text>
        </View>
        ...
        <View style={s.statsGrid}>
          {[
            { label: 'Note', value: String(activity.rating), icon: 'star' },
            { label: 'Prix', value: activity.price, icon: 'tag' },
            { label: 'Duree', value: activity.duration, icon: 'clock' },
            { label: 'XP', value: `+${activity.xp}`, icon: 'award', isXp: true },
          ].map((stat, i) => (
